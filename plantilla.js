...
document.addEventListener("DOMContentLoaded", function() { 
    ...
    // Filtrar jugadores del club que no están vendidos ni cedidos ni retirados
    ...
    jugadoresDelClub.forEach(jugador => {
        ...
        // **MODIFICACIÓN: Mostrar información del contrato adaptada al calendario del juego**
        let infoContrato = '';
    ...
    const jugadoresCedidosDelClub = jugadoresCedidos.filter(cedido => {
        ...
        return jugadorCompleto && jugadorCompleto.club === selectedClub && !jugadorCompleto.retirado;
    });
...
function verificarContratosVencidos(jugadores) {
    ...
    for (let i = jugadores.length - 1; i >= 0; i--) {
        ...
        if (jugador.contrato && jugador.contrato.fechaVencimiento) {
            ...
            // Para jugadores con el sistema antiguo, verificar también
            const fechaInicial = new Date("2025-01-01");
            const fechaVencimiento = new Date(fechaInicial);
...
// Función para evaluar retiro de jugadores
