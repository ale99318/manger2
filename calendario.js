...
// **FUNCIÓN MODIFICADA: Verificar contratos sin eliminar jugadores**
function verificarContratos() {
    ...
    jugadores.forEach(jugador => {
        ...
        if (jugador.contrato && jugador.contrato.fechaVencimiento) {
            ...
            if (diasParaVencer < 0) {
                // Contrato vencido - solo reportar, no eliminar
                contratosVencidos.push({
                    jugador: jugador,
                    diasVencido: Math.abs(diasParaVencer)
                });
                
                console.log(`⚠️ Contrato vencido: ${jugador.nombre} (hace ${Math.abs(diasParaVencer)} días)`);
        ...
        if (contratosVencidos.length > 0) {
            ...
            contratosVencidos.forEach(item => {
                ...
                div.innerHTML = `
                    ...
                    <p>⚠️ CONTRATO VENCIDO - hace ${item.diasVencido} día(s)</p>
